...
Interpret client `model` as a routing intent tier (fast-thinking for nano/mini,
deep for plain gpt-5) instead of strict `model-mapping.yaml` binding. Autodefault
selects from the global credential×model pool with payload filters, preferred-tier
ranking, and asymmetric escalation.
