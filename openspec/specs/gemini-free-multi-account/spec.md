...
Spread autodefault Gemini traffic across up to sixteen free-tier Google AI Studio
API keys with per-slot secrets resolution, round-robin, and isolated
cooldown/failover before falling back to paid `gemini-default` or other providers.

## Requirements

### Requirement: Sixteen free-tier Gemini credential slots
