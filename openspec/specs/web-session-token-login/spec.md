# web-session-token-login

## Purpose

Extend the shared headed-browser login flow so web providers whose credential is
a page-scoped bearer token (not a cookie) can authenticate through the same
operator workflow.
