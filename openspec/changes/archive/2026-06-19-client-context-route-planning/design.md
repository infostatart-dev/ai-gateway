...
### D8 — Test architecture (`routing_load` + `gateway-tests`)

**Industrial rule (v1.1):** routing and failover tests MUST use the workspace crate
`crates/gateway-tests` — declarative `UpstreamMockScript` keyed by
`(credential_id, model_slug)`, not implicit FIFO `push` ordering.

| Layer | Location | Responsibility |
|-------|----------|----------------|
| Shared mocks | `crates/gateway-tests` | `UpstreamMockScript`, response factories, `install_upstream_mock` |
| Router hook | `budget_aware/call.rs` | `pop(credential, model)` — script first, legacy FIFO deprecated |
| Test facade | `ai-gateway/src/tests/routing.rs` + `routing_harness/` | `run_planned_failover`, headers, assert_stats (feature `testing` only) |
| Scenarios | `ai-gateway/tests/rl/scenarios/` | Domain steps + trace/stats assertions |
| Registration | `ai-gateway/tests/routing_load.rs` | `#[serial_test::serial]` integration test binary |
| Unit | `ai-gateway/tests/budget_aware_*.rs` | Pure planner/memory without HTTP |

**Declarative mock (required for new scenarios):**

```rust
install_upstream_mock(
    UpstreamMockScript::new()
        .binding("gemini-free-9", "gemini-3.1-flash-lite", vec![ok, rpm_429])
        .credential("gemini-free-10", vec![ok])
);
```

**Forbidden in new tests:**

- Sequencing behavior via N× `push_test_call_response_for_credential` without
  documenting which hop each push serves
- Asserting routing outcomes without `route_memory_invalidated` / stats when spec
  requires observability

**Deprecated (migration only):** `push_test_call_response*` global/per-credential FIFO.
Existing scenarios migrate incrementally; new scenarios MUST NOT add FIFO-only scripts.
...
| `route_memory_invalidate_on_429.rs` | 429 on binding invalidates; failover hop | 2 |
...
- `install_upstream_mock(UpstreamMockScript)` — declarative per-hop responses
- `RoutingLoadProfile::gemini_free_slots(n)` — already exists via harness

Unit tests mirror scenarios in `ai-gateway/tests/budget_aware_plan.rs`,
`budget_aware_memory.rs`, and `budget_aware_snapshot.rs` (not under `src/`).

Every workspace library crate lists `gateway-tests` as `dev-dependency` for shared fixtures.
