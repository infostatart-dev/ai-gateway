...
impl ProviderMonitorInner<ModelWeightedKey> {
    fn create_model_weighted_key(&self, event: &RateLimitEvent) -> Result<ModelWeightedKey, InternalError> {
        ...
        let weight = if let Some(BalanceConfigInner::ModelWeighted { models }) = self.router_config.load_balance.0.get(&endpoint_type) {
        ...
        let weight = Weight::from(weight.to_f64().ok_or(InternalError::Internal)?);
    ...
    pub async fn monitor(self, mut rx: Receiver<RateLimitEvent>) -> Result<(), RuntimeError> {
        ...
        let mut pending_restores: FuturesUnordered<ProviderRestore<ModelWeightedKey>> = FuturesUnordered::new();
