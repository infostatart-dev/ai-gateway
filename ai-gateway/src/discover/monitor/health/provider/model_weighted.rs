...
pub async fn check_model_weighted_monitor(inner: &mut ProviderMonitorInner<ModelWeightedKey>) -> Result<(), RuntimeError> {
        ...
        match balance_config {
            BalanceConfigInner::ModelWeighted { models } => {
                for target in models {
                    ...
                    let weight = Weight::from(target.weight.to_f64().ok_or_else(|| InitError::InvalidWeight(provider.clone()))?);
                    ...
                    if !is_healthy && !was_unhealthy {
                        ...
                        let all_models = models.iter().filter(|m| m.model.inference_provider().as_ref() == Some(&provider)).collect::<Vec<_>>();
                        let mut join_set = JoinSet::new();
                        for unhealthy_model in all_models {
                            let weight = Weight::from(unhealthy_model.weight.to_f64().ok_or_else(|| InitError::InvalidWeight(provider.clone()))?);
                            let unhealthy_key = ModelWeightedKey::new(unhealthy_model.model.clone(), *endpoint_type, weight);
                        ...
                        for healthy_model in all_models {
                            let weight = Weight::from(healthy_model.weight.to_f64().ok_or_else(|| InitError::InvalidWeight(provider.clone()))?);
                            let key = ModelWeightedKey::new(healthy_model.model.clone(), *endpoint_type, weight);
