...
pub async fn check_provider_weighted_monitor(inner: &mut ProviderMonitorInner<ProviderWeightedKey>) -> Result<(), RuntimeError> {
        ...
        match balance_config {
            BalanceConfigInner::ProviderWeighted { providers } => {
                for target in providers {
                    ...
                    let weight = Weight::from(target.weight.to_f64().ok_or_else(|| InitError::InvalidWeight(target.provider.clone()))?);
                    ...
                    if !is_healthy && !was_unhealthy {
                        ...
                        if let Err(e) = inner.tx.send(Change::Remove(key.clone())).await { error!(error = ?e, "Failed to send remove event for unhealthy provider"); }
                    ...
                    inner.app_state.0.metrics.provider_health.record(if is_healthy { 1 } else { 0 }, &metric_attributes);
                }
            }
