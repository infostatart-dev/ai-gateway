...
use crate::{
    ...
    router::{
        budget_aware::{
            balance_ranked, clear_test_call_responses, deep_paid_candidate,
            intent_autodefault_router, ordered_candidates_for_source,
            push_test_call_response, run_failover_candidates, scout_candidate,
        },
        capability::{
            extract_requirements_from_value, extract_source_model_from_value,
        },
        intent::{IntentTier, extract_routing_intent},
    },
    routing_load::{
        assert_identity::{
...
pub async fn run() {
    ...
    for _ in 0..32 {
        push_test_call_response(Ok(ok_nano_json_schema_completion()));
    }
    ...
    let parts = crate::router::budget_aware::request_parts();
