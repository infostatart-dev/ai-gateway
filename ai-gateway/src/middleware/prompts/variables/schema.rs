...
use super::{utils::{is_whole_variable_match, get_variable_name_from_string}, replace::replace_variables};

pub fn process_prompt_schema(value: serde_json::Value, inputs: &HashMap<String, serde_json::Value>, regex: &Regex) -> Result<serde_json::Value, ApiError> {
    match value {
        serde_json::Value::String(s) => {
            if is_whole_variable_match(&s, regex) {
                let name = get_variable_name_from_string(&s, regex)?;
                if let Some(val) = inputs.get(&name) { return Ok(val.clone()); }
            }
            Ok(serde_json::Value::String(replace_variables(&s, inputs, regex, &mut HashSet::new())?))
        }
        serde_json::Value::Array(arr) => {
            let mut processed = Vec::new();
            for item in arr { processed.push(process_prompt_schema(item, inputs, regex)?); }
        ...
        serde_json::Value::Object(obj) => {
            ...
            for (key, val) in obj {
                ...
                } else { replace_variables(&key, inputs, regex, &mut HashSet::new())? };
                processed.insert(p_key, process_prompt_schema(val, inputs, regex)?);
            }
