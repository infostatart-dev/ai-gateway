...
pub struct DeepSeekWebChatCompletions;

impl Endpoint for DeepSeekWebChatCompletions {
...
#[derive(Debug, Clone)]
pub struct DeepSeekWebConverter {
    model_mapper: ModelMapper,
}

impl DeepSeekWebConverter {
    #[must_use]
...
impl TryConvert<Value, CreateChatCompletionResponse> for DeepSeekWebConverter {
    type Error = MapperError;
    fn try_convert(
        &self,
        mut value: Value,
...
mod tests {
    ...
    #[tokio::test]
    async fn maps_bare_openai_model_to_deepseek_web_binding() {
        let app_state = AppState::test_default().await;
        let converter = DeepSeekWebConverter::new(ModelMapper::new(app_state));

        let mapped = converter
            .try_convert(chat_request("gpt-5-mini"))
            .expect("bare openai model should map");

        assert_eq!(mapped.inner.model, "deepseek-chat");
    }
}
