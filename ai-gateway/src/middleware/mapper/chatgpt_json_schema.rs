//! Prompt injection for ChatGPT web — OpenAI `response_format` has no native
//! equivalent on chatgpt.com, so we prepend schema instructions to the system message.
...
const ONLY_JSON_TAIL: &str =
    "Output ONLY the JSON object in the message content. No markdown fences, no prose.";

const STRICT_MANDATORY: &str = "MANDATORY strict mode: you MUST NOT omit any required field, \
    use wrong types, or add extra properties. The response must be a single JSON object that \
    validates against the schema below — no exceptions.";

/// Builds the upstream instruction when structured output is requested.
#[must_use]
pub fn build_json_schema_instruction(format: &ResponseFormat) -> Option<String> {
    let schema_text = match format {
        ResponseFormat::JsonSchema { json_schema } => {
            ...
            let schema = json_schema
                .schema
                .as_ref()
                .map(|s| serde_json::to_string_pretty(s).unwrap_or_else(|_| "{}".into()))
                .unwrap_or_else(|| "{}".into());
            lines.push(format!(
...
mod tests {
    ...
    fn injects_new_system_message_when_missing() {
        ...
        let ChatCompletionRequestSystemMessageContent::Text(text) = &sys.content else {
