...
impl TryConvertStreamData<anthropic::StreamEvent, openai::CreateChatCompletionStreamResponse> for super::AnthropicConverter {
    type Error = MapperError;

    #[allow(deprecated)]
    fn try_convert_chunk(&self, value: anthropic::StreamEvent) -> Result<Option<openai::CreateChatCompletionStreamResponse>, Self::Error> {
        ...
        match value {
            anthropic::StreamEvent::MessageStart { message } => {
                ...
                for (idx, content_block) in message.content.iter().enumerate() {
                    match content_block {
                        anthropic::ContentBlock::Text { text, .. } => current_text_content.push_str(text),
                        anthropic::ContentBlock::ToolUse { id, name, input } => {
                            tool_calls.push(openai::ChatCompletionMessageToolCallChunk {
                                index: idx as u32,
                                id: Some(id.clone()),
                                r#type: Some(openai::FunctionType::Function),
                                function: Some(openai::FunctionCallStream {
                                    name: Some(name.clone()),
                                    arguments: Some(serde_json::to_string(input).map_err(MapperError::SerdeError)?),
                                }),
                            });
                        }
                ...
                let finish_reason = match message.stop_reason {
                    Some(anthropic::StopReason::EndTurn | anthropic::StopReason::StopSequence) => Some(openai::FinishReason::Stop),
                    ...
                    Some(anthropic::StopReason::Refusal) => Some(openai::FinishReason::ContentFilter),
                    None => None,
                };
                ...
                let choice = openai::ChatChoiceStream {
                    ...
                    delta: openai::ChatCompletionStreamResponseDelta {
                        ...
                        refusal: if matches!(message.stop_reason, Some(anthropic::StopReason::Refusal)) { message.stop_sequence.clone() } else { None },
                        function_call: None,
                    },
                    finish_reason,
                    logprobs: None,
            ...
            anthropic::StreamEvent::ContentBlockStart { index, content_block } => {
                if let anthropic::ContentBlock::ToolUse { id, name, input } = content_block {
                    ...
                    let choice = openai::ChatChoiceStream {
                        index: 0,
                        delta: openai::ChatCompletionStreamResponseDelta {
                            role: None, content: None, tool_calls: Some(vec![tool_call_chunk]), refusal: None, function_call: None,
                        },
                        finish_reason: None,
                        logprobs: None,
                    };
            ...
            anthropic::StreamEvent::ContentBlockDelta { index, delta } => {
                let delta_msg = match delta {
                    anthropic::ContentBlockDelta::TextDelta { text } => {
                        openai::ChatCompletionStreamResponseDelta {
                            role: None, content: Some(text), tool_calls: None, refusal: None, function_call: None,
                        }
                    }
                    anthropic::ContentBlockDelta::InputJsonDelta { partial_json } => {
                        openai::ChatCompletionStreamResponseDelta {
                            role: None, content: None, refusal: None, function_call: None,
                            tool_calls: Some(vec![openai::ChatCompletionMessageToolCallChunk {
                                index: index as u32,
                                id: None,
                                r#type: Some(openai::FunctionType::Function),
                                function: Some(openai::FunctionCallStream { name: None, arguments: Some(partial_json) }),
                            }]),
                        }
                    }
                    _ => return Ok(None),
                ...
                Ok(Some(openai::CreateChatCompletionStreamResponse {
                    id: PLACEHOLDER_STREAM_ID.to_string(),
                    choices: vec![openai::ChatChoiceStream { index: 0, delta: delta_msg, finish_reason: None, logprobs: None }],
                    created: 0,
                    model: PLACEHOLDER_MODEL_NAME.to_string(),
                    object: CHAT_COMPLETION_CHUNK_OBJECT.to_string(),
                    system_fingerprint: None,
            ...
            anthropic::StreamEvent::MessageDelta { delta, usage } => {
                ...
                Ok(Some(openai::CreateChatCompletionStreamResponse {
                    ...
                    choices: vec![openai::ChatChoiceStream {
                        index: 0,
                        delta: openai::ChatCompletionStreamResponseDelta { role: None, content: None, tool_calls: None, refusal: delta.stop_sequence, function_call: None },
                        finish_reason,
                        logprobs: None,
                    }],
                    created: 0,
...
impl TryConvertStreamData<anthropic::StreamEvent, anthropic::StreamEvent> for super::AnthropicConverter {
    type Error = MapperError;
    fn try_convert_chunk(&self, value: anthropic::StreamEvent) -> Result<Option<anthropic::StreamEvent>, Self::Error> {
        Ok(Some(value))
    }
}
