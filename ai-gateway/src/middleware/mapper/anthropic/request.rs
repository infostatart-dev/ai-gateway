...
use crate::{
    ...
    types::{model_id::{ModelId, Version}, provider::InferenceProvider},
};
...
impl TryConvert<openai::CreateChatCompletionRequest, anthropic::CreateMessageParams> for AnthropicConverter {
    type Error = MapperError;

    fn try_convert(&self, value: openai::CreateChatCompletionRequest) -> Result<anthropic::CreateMessageParams, Self::Error> {
        let source_model = ModelId::from_str(&value.model)?;
        let mut target_model = self.model_mapper.map_model(&source_model, &InferenceProvider::Anthropic)?;

        if let ModelId::ModelIdWithVersion { provider: InferenceProvider::Anthropic, id: model } = &mut target_model {
        ...
        let max_tokens = value.max_completion_tokens
            .or(#[allow(deprecated)] value.max_tokens)
        ...
        let user = value.prompt_cache_key
            .or(value.safety_identifier)
            .or(#[allow(deprecated)] value.user);
        ...
        for message in value.messages {
            match message {
                openai::ChatCompletionRequestMessage::Developer(_) | openai::ChatCompletionRequestMessage::System(_) => {}
                ...
                openai::ChatCompletionRequestMessage::Assistant(msg) => {
                    ...
                    if !content_blocks.is_empty() {
                        mapped_messages.push(anthropic::Message {
                            ...
                            content: anthropic::MessageContent::Blocks { content: content_blocks },
                        });
                    }
                }
...
impl TryConvert<anthropic::CreateMessageParams, anthropic::CreateMessageParams> for AnthropicConverter {
    type Error = MapperError;
    fn try_convert(&self, mut value: anthropic::CreateMessageParams) -> Result<anthropic::CreateMessageParams, Self::Error> {
        let source_model = ModelId::from_str(&value.model)?;
        let target_model = self.model_mapper.map_model(&source_model, &InferenceProvider::Anthropic)?;
