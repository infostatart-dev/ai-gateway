...
use crate::endpoints::bedrock::converse::{Tool, ToolChoice, SpecificToolChoice, ToolSpecification, ToolInputSchema};

pub fn map_tools(tools: Option<Vec<openai::ChatCompletionTools>>) -> Option<Vec<Tool>> {
    tools.map(|ts| ts.into_iter().filter_map(|t_enum| {
...
pub fn map_tool_choice(choice: Option<openai::ChatCompletionToolChoiceOption>) -> Option<ToolChoice> {
    match choice {
        Some(openai::ChatCompletionToolChoiceOption::Function(tool)) => Some(ToolChoice::Tool {
            tool: SpecificToolChoice { name: tool.function.name },
        }),
