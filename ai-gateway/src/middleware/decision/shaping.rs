...
impl TrafficShaper {
    ...
    /// Acquire combined permit for the given tier (без cascade).
    ...
    /// Acquire с tier-каскадом: при exhausted на стартовом тире пробует
    /// следующий по cascade-порядку.
    /// Каждая попытка использует тот же `timeout`. Возвращает первую успешную.
...
/// Result of cascade acquire — какой тир в итоге получили + permit.
pub struct AcquireOutcome {
    pub tier: Tier,
    pub permit: CombinedPermit,
}

/// Возвращает упорядоченный список тиров, начиная со `start`, согласно
/// cascade-режиму. Для `OnlyTier` — список из одного тира.
fn cascade_chain(start: Tier, cascade: TierCascade) -> Vec<Tier> {
    match cascade {
        TierCascade::OnlyTier => vec![start],
        TierCascade::PaidDown => {
            // paid → freemium → free, отрезанный по `start`.
...
mod tests {
    ...
    async fn acquire_with_cascade_falls_back_to_next_tier() {
        // free тир насыщен (limit=1, держим permit), freemium свободен.
