    ...
    async fn authenticate(
        ...
        match self {
            Client::Bedrock(inner) => {
                inner.extract_and_sign_aws_headers(rb, body)
            }
            Client::OpenAICompatible(_) | Client::Anthropic(_) => {
                self.authenticate_inner(app_state, rb, auth, provider).await
            }
...
impl Client {
    async fn authenticate_inner(
        ...
        let key = if let Some(ProviderKey::Secret(k)) = app_state
            .0
            .provider_keys
            ...
            && k.expose() != ""
        {
            ...
            if let Some(ProviderKey::Secret(k)) = keys.get(&provider) {
                k.clone()
            } else {
        ...
        Ok(match self {
            Client::OpenAICompatible(_) => {
                OpenAICompatibleClient::set_auth_header(rb, &key)
            }
