...
use super::{DatabaseListener, types::{ConnectedCloudGatewaysNotification, Op}};

impl DatabaseListener {
    #[allow(clippy::too_many_lines)]
    pub async fn handle_notification(&mut self, notification: &sqlx::postgres::PgNotification, tx: Sender<Change<RouterId, Router>>) -> Result<(), RuntimeError> {
        ...
        if notification.channel() == "connected_cloud_gateways" {
            ...
            match payload {
                ConnectedCloudGatewaysNotification::RouterConfigUpdated { router_hash, organization_id, op, config, .. } => {
                    info!(router_hash = %router_hash, organization_id = %organization_id, "router configuration created/updated");
                    match op {
                        Op::Insert => {
                            self.app_state.increment_router_metrics(&router_hash, &config, Some(organization_id));
                            ...
                            self.last_router_config_versions.insert(router_hash.to_string(), Utc::now());
                            Ok(())
                        }
                        Op::Delete => {
                            self.app_state.decrement_router_metrics(&router_hash, &config, Some(organization_id));
                            ...
                            self.last_router_config_versions.remove(&router_hash.to_string());
                            Ok(())
                        }
                        _ => { debug!("skipping router insert"); Ok(()) }
                    }
                }
                ConnectedCloudGatewaysNotification::ApiKeyUpdated { owner_id, organization_id, api_key_hash, soft_delete, op } => match op {
                    Op::Insert => {
                        ...
                        self.last_api_key_created_at.insert(api_key_hash, Utc::now());
                        Ok(())
                    }
                    Op::Delete => {
                    ...
                    Op::Update => {
                        if soft_delete { self.app_state.remove_helicone_api_key(api_key_hash.clone()).await.map_err(|e| { error!(error = %e, "failed to remove helicone api key"); e })?; info!(owner_id = %owner_id, organization_id = %organization_id, "helicone api key soft deleted"); self.last_api_key_created_at.remove(&api_key_hash); }
                        else { self.last_api_key_created_at.insert(api_key_hash, Utc::now()); }
                        Ok(())
                    }
                    Op::Truncate => { debug!("skipping helicone api key truncate"); Ok(()) }
                },
                ConnectedCloudGatewaysNotification::Unknown { data } => { debug!("Unknown notification event: {:?}", data); Ok(()) }
            }
        } else { debug!("received unknown notification"); Ok(()) }
    }
}
