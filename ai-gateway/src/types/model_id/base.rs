...
impl FromStr for ModelIdWithVersion {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        if s.is_empty() { return Err(MapperError::InvalidModelName("Model name cannot be empty".to_string())); }
        if s.ends_with('-') || s.ends_with('.') || s.ends_with('@') {
            return Err(MapperError::InvalidModelName(format!("Model name cannot end with {}", s.chars().last().unwrap())));
        }
        let (model, version) = parse_model_and_version(s, '-');
        Ok(ModelIdWithVersion { model: model.to_string(), version: version.unwrap_or(Version::ImplicitLatest) })
    }
}
