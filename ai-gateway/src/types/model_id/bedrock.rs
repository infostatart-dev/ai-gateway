...
impl FromStr for BedrockModelId {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let (geo, provider, rest) = if dot_count >= 2 {
            ...
            (Some(parts.next().unwrap().to_string()), parts.next().unwrap(), parts.next().unwrap())
        } else if dot_count == 1 {
            let mut parts = s.splitn(2, '.');
            (None, parts.next().unwrap(), parts.next().unwrap())
        } else { return Err(MapperError::InvalidModelName(s.to_string())); };
        ...
        Ok(BedrockModelId { geo, provider: provider.to_string(), model: model.to_string(), 
            version: version.unwrap_or(Version::ImplicitLatest), bedrock_internal_version: bedrock_version.to_string() })
    }
}

impl Display for BedrockModelId {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let geo_str = self.geo.as_ref().map(|g| format!("{g}.")).unwrap_or_default();
