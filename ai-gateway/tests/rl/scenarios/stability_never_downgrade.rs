...
fn never_downgrade_script() -> UpstreamMockScript {
...
pub async fn run() {
    ...
    install_upstream_mock(never_downgrade_script());
    ...
    let planned = run_planned_failover(
        router,
        caller_parts("stability-up", Some("unit-up")),
        default_fat_body(),
        pool,
    ...
    assert_eq!(
        ...
        before_or,
        "nemotron must not be attempted when gemini-2.5-flash-lite succeeds"
    );
}
