...
use ai_gateway::{
    ...
    tests::{
        ...
        routing_harness::headers::caller_parts_with_request_id,
    },
};
...
pub async fn run() {
    ...
    let futures = (1..=3).map(|unit| {
        ...
        async move {
            run_planned_failover(
                router,
                caller_parts_with_request_id(AGENT, &format!("req-{unit}")),
                body,
                pool,
    ...
    assert!(
        healthy.len() >= 2,
        "expected spread across healthy credentials via request-id work \
         units, got {healthy:?}; identities={}",
        results
            .iter()
